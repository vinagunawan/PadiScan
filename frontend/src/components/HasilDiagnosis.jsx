    ...
    <div className="bg-utama">
      ...
      <section className="container py-5">
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 3 }}>
          <Card sx={{ maxWidth: 600, width: '100%', padding: 3, borderRadius: '8px', boxShadow: 0, backgroundColor: 'transparent' }}>
            <CardContent>
              {image ? (
                <img src={image} alt="Diagnosis Result" style={{ width: '100%', height: 'auto', borderRadius: '8px', marginBottom: '16px' }} />
              ) : (
                <Typography variant="body2" className="text-center">Gambar tidak ditemukan.</Typography>
              )}
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', marginTop: 2, gap: 3, width: '100%' }}>
                <button className="btn btn-secondary w-100" onClick={handleHistory}>History</button>
                <button className="btn btn-success w-100" onClick={handleDiagnosisUlang}>Diagnosis Ulang</button>
              </Box>
            </CardContent>
          </Card>
        </Box>
